import { conciseCreateSelectionBlock, createSelectionBlock } from "./create_select";
...
describe("RUn without issue", () => {
  ...
  it("Should run", () => {
    console.log(JSON.stringify(conciseCreateSelectionBlock(
      ["iss"],
      ["Project1", "Project2"])))
  });
  ...
  it("Should run with test0", () => {
    let empty = conciseCreateSelectionBlock([], []);

    console.log(JSON.stringify(empty));
  })
  it("Should run with test00", async () => {  
    await testPostToSlack(eventResURL, conciseCreateSelectionBlock(["JIUsbds", "Iodshs"], ["Project0", "Project1", "Project clear", "Anime", "Writter"]));
  });
