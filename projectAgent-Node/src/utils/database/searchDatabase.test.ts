...
describe("Tests filterSimilar on the message 'Find the cats in the dev environment'", () => {
  it("Should find all tasks involving cats or dev environments", () => {
    ...
    expect(similarPages).toContain(similarPages.find(page => page.pageId === FEED_ANIMALS_ID));
    const FEED_CAT_ID = "2e0eef29-a653-817d-9689-f8d338025ab4";
    expect(similarPages).toContain(similarPages.find(page => page.pageId === FEED_CAT_ID));
    const FEED_CATS_DOG_FOOD_ID = "2e0eef29-a653-81a4-84cb-d512772d0b30";
    expect(similarPages).toContain(similarPages.find(page => page.pageId === FEED_CATS_DOG_FOOD_ID));
    const PET_CATS_ID = "2e0eef29-a653-81b1-b6db-ca7af7a44081";
    expect(similarPages).toContain(similarPages.find(page => page.pageId === PET_CATS_ID));
    const FEED_CATS_TWICE_A_DAY_ID = "2e2eef29-a653-81b8-a569-e79f565fffee";
    expect(similarPages).toContain(similarPages.find(page => page.pageId === FEED_CATS_TWICE_A_DAY_ID));
    const SET_UP_NEW_DEV_ENVIRONMENT_ID = "2e1eef29-a653-815d-b9f9-de045fd62cdd";
    expect(similarPages).toContain(similarPages.find(page => page.pageId === SET_UP_NEW_DEV_ENVIRONMENT_ID));
...
describe("Tests filterSimilar on the message 'Josh, row the new boat'", () => {
  it("Should find all tasks that involve rowing boats", () => {
    ...
    expect(similarPages).toContain(similarPages.find(page => page.pageId === ROW_BOAT_ID));
