...
describe("Tests parseTaskSlashCmd with a good payload", () => {
  it("Parses the task correctly", async () => {
    ...
    const parsedTask = await parseTask(payloadGood, payloadGood.text, timestamp, EXAMPLE_RAW_PROJECTS_RESPONSE);
...
describe("Tests parseTaskSlashCmd with a good payload from Harvey", () => {
  it("Parses the task correctly", async () => {
    ...
    const parsedTask = await parseTask(payloadHarvey, payloadHarvey.text, timestamp, EXAMPLE_RAW_PROJECTS_RESPONSE);
    console.log(JSON.stringify(parsedTask));
...
describe("Tests parseTaskSlashCmd inferring dates", () => {
  it("Parses the task and infers start date and due date correctly", async () => {
    ...
    const parsedTask = await parseTask(payloadInferDates, payloadInferDates.text,timestamp, EXAMPLE_RAW_PROJECTS_RESPONSE);
    console.log(JSON.stringify(parsedTask));
    expect(parsedTask.task.taskTitle).toBeTruthy();
    expect(parsedTask.task.description).toBeTruthy();
  }, 20000);
...
describe("tests parseTaskSlashCmd with the payload that's been causing trouble", () => {
  it("", async () => {
    ...
    const parsedTask = await parseTask(payloadNew, payloadNew.text,timestamp, EXAMPLE_RAW_PROJECTS_RESPONSE);
    console.log(JSON.stringify(parsedTask));
