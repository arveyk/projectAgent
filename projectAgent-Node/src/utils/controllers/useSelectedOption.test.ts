...
describe("Tests that selected values in payload are added to task object", () => {
  ...
  it("Should extract users's assignee selections", () => {
      ...
      expect(allAssignees.length).toBeGreaterThan(taskPageObjAssigneeSelected.task.assignees.length);
      expect(taskWithAdditionalAssignees.project?.length).toBe(taskPageObjAssigneeSelected.task.project 
  ...
  it("Should extract users's project selections", () => {
      ...
      expect(taskPageObject.task.assignees).toEqual(taskWithAdditionalProjects.assignees);
      expect(taskWithAdditionalProjects.project?.length).toBeGreaterThan(taskPageObject.task.project 
        ? taskPageObject.task.project.length
        : 0
      )
    });
