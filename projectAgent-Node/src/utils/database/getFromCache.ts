 ...
 */
export async function extractCacheData(
  ...
  const projects: QueryDataSourceResponse["results"] | ListUsersResponse =
    await extractCacheItemData(projectsRecord);
  const tasks: QueryDataSourceResponse["results"] | ListUsersResponse =
    await extractCacheItemData(tasksRecord);
  const users: QueryDataSourceResponse["results"] | ListUsersResponse =
    await extractCacheItemData(usersRecord);
 ...
 */
export async function extractCacheItemData(
  itemRecord: Record<string, any> | undefined,
) {
  const itemCompressed: Buffer<ArrayBuffer> | null = itemRecord
    ? itemRecord["RawResponse"]
    : null;

  const itemData: QueryDataSourceResponse["results"] | ListUsersResponse =
    itemCompressed
      ? JSON.parse((await gunzipPromise(itemCompressed)).toString())
      : null;
  return itemData;
}
