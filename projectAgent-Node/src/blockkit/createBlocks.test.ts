...
describe("Test createBlockNewTask with a valid task object", () => {
  it("returns blocks containing the task data", () => {
    ...
    expect(JSON.stringify(slackBlocksObject.blocks)).toMatch(
      /Assignees:.{1,4}Jacob \(jacomsmail@example.com\)/gm,
    );
    expect(JSON.stringify(slackBlocksObject.blocks)).toMatch(dueDateRegex);
    expect(JSON.stringify(slackBlocksObject.blocks)).toMatch(startDateRegex);
    expect(JSON.stringify(slackBlocksObject.blocks)).toMatch(
